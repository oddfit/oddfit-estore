...
type Order = {
  ...
  createdAt?: any;
  updatedAt?: any;
  // ⬇️ Added (used by OrderConfirmationPage already)
...
const AdminOrders: React.FC = () => {
  ...
  // ⬇️ New state for tracking fields (only used when status === 'shipped')
      ...
      setRows(
        list.map((r: any) => {
          ...
          return {
            ...
            // ⬇️ Pull existing values if present
  ...
  const openEdit = (o: Order) => {
    ...
    // ⬇️ seed tracking fields (if they already exist on the order)
    setEditTrackingNumber(o.trackingNumber || '');
    setEditTrackingUrl(o.trackingUrl || '');
    setEditOpen(true);
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // ⬇️ Only write tracking fields if status is shipped (per your requirement)
      if (editStatus === 'shipped') {
        patch.trackingNumber = editTrackingNumber || null;
        patch.trackingUrl = editTrackingUrl || null;
      }

      await AdminOrdersService.update(editing.id, patch);
            ...
            <tbody>
              {sorted.map((o) => (
                <tr key={o.id} className="border-b last:border-0">
                  <td className="py-2 pr-4 font-mono">{(o as any).orderCode || `${o.id.slice(0,8)}…`}</td>
      ...
      >
        {!editing ? null : (
          <div className="space-y-4">
            ...
            {/* ⬇️ Only show these when the order is shipped */}
            {editStatus === 'shipped' && (
              <>
                <Input
